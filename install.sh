...
do
  echo -n "."
  sleep 5
  DNRN=$(kubectl -n ${K8S_HDFS_NAMESPACE} get pods --no-headers -l app=hdfs-datanode,release=${K8S_HDFS_NAME} | grep -E '1\/\w\ *Running')
  echo -n "="
done
...
do
  HDFS_CLIENT_POD=$(kubectl -n ${K8S_HDFS_NAMESPACE} get pods --no-headers -l app=hdfs-client,release=${K8S_HDFS_NAME} | grep -E '1\/\w\ *Running' | awk '{print $1}')
  sleep 5
done
...
CLIENT_POD=$(kubectl -n ${K8S_HDFS_NAMESPACE} get pods --no-headers -l app=hdfs-client,release=${K8S_HDFS_NAME} -o name)
...
HDFS_NAMENODE_POD="$(kubectl -n ${K8S_HDFS_NAMESPACE} get pods --no-headers -l app=hdfs-namenode,release=${K8S_HDFS_NAME} -o jsonpath='{.items[0].metadata.name}')"
HDFS_NAMENODE_SERVICE="$(kubectl -n ${K8S_HDFS_NAMESPACE} get services --no-headers -l app=hdfs-namenode,release=${K8S_HDFS_NAME} -o jsonpath='{.items[0].metadata.name}')"
...
git clone https://github.com/neoflex-consulting/datagram.git ${DGTMP}
